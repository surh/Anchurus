  ...
  if(ncol(Dat$midas$freq) > 2){
    ...
    Res <- Dat$midas$info %>%
      ...
      map(function(i, freq, depth, map, depth_thres, freq_thres, min_cov,
                   genome_feats, missing_as, genome_fasta, outdir){
        
        # i <- Dat$midas$info %>% filter(gene_id == "686660.3.peg.150")
        # freq <- Dat$midas$freq
        # depth <- Dat$midas$depth
        # map <- map
        # depth_thres <- args$depth_thres
        # freq_thres <- args$freq_thres
        # min_cov <- args$min_cov
        # genome_feats <- Dat$genome_feats
        # missing_as <- args$missing_as
      ...
      genome_fasta=Dat$genome_fasta, outdir = args$outdir)
  }else{
